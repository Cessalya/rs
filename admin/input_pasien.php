...
$hasil = mysqli_query("SELECT MAX(kode_pasien) AS kode FROM pasien");

$maks = mysqli_fetch_array($hasil); //memecah hasil ke dalam array
...
$query= mysqli_query("SELECT MAX(kode_inap) AS kode_inap FROM rawat_inap");
$inap= mysqli_fetch_array($query);
$kode_inap_baru = $inap['kode_inap']+1;

//mengambil data max dari tabel ruangan untuk dimasukkan secara otomatis ke form
$query2 = mysqli_query("SELECT MAX(kode_ruangan) AS kode_ruang FROM ruangan");
$ruang = mysqli_fetch_array($query2);
