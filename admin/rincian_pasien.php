...
$hasil = mysqli_query("SELECT * FROM pasien WHERE kode_pasien = '$kode_pasien'");//mengambil data pasien

//mengambil nama dokter yang menangani pasien
$hasil2 = mysqli_query("SELECT nama_dokter FROM dokter WHERE kode_dokter IN (SELECT kode_dokter FROM tgl_masuk WHERE kode_pasien='$kode_pasien')");

//mengambil nama suster perawat
$hasil3 = mysqli_query("SELECT nama_suster FROM suster WHERE kode_dokter IN (SELECT kode_dokter FROM tgl_masuk WHERE kode_pasien='$kode_pasien')");

//mengambil tanggal masuk
$hasil4 = mysqli_query("SELECT tgl_masuk FROM tgl_masuk WHERE kode_pasien='$kode_pasien'");

//mengambil rincian ruangan
$hasil5 = mysqli_query("SELECT * FROM ruangan WHERE kode_pasien='$kode_pasien'");

//mengambil data pembayaran
$hasil7 = mysqli_query("SELECT lama_rawat, jmlh_pembayaran FROM pembayaran WHERE kode_pembayaran='$kode_pasien'");

//mengambil tanggal keluar
$hasil6 = mysqli_query("SELECT tgl_keluar FROM tgl_keluar WHERE kode_pasien='$kode_pasien'");

//memecah ke dalam array dari tabel pasien
$pasien = mysqli_fetch_array($hasil);

//memecah ke dalam array dari tabel tgl_masuk
$tgl_masuk = mysqli_fetch_array($hasil4);

//memecah ke dalam array dari tabel ruangan
$ruangan = mysqli_fetch_array($hasil5);
?>
...
<input name="kode_ruangan" type="hidden" value="<?php echo $ruangan[kode_ruangan]?>"/>
  ...
  <tr>
    ...
    <td>:</td>
    <td><?php $dokter = mysqli_fetch_array($hasil2); echo "$dokter[nama_dokter] ";?><a href="ubah_dokter.php?kodepasien=<?php echo $kode_pasien?>"><img src="../icon/update.png" border="0"></a>    </td>
  </tr>
  <tr>
    <td>Perawat</td>
    <td>:</td>
    <td><?php $suster = mysqli_fetch_array($hasil3); echo $suster['nama_suster']; ?></td>
  </tr>
  <tr>
    <td>Tanggal Keluar </td>
    <td>:</td>
    <td><input type="text" name="tgl_keluar" size="10" value="<?php $tgl_keluar = mysqli_fetch_array($hasil6); echo $tgl_keluar['tgl_keluar'] ?>"/><a href="javascript:void(0)" onClick="if(self.gfPop)gfPop.fPopCalendar(document.postform.tgl_keluar);return false;" ><img name="popcal" align="absmiddle" style="border:none" src="./calender/calender.jpeg" width="34" height="29" border="0" alt=""></a></td>
  </tr>
  <tr>
    <td>Lama Rawat Inap </td>
    <td>:</td>
    <td><select name="lama_rawat">
	<?php $pembayaran=mysqli_fetch_array($hasil7);echo "<option selected=selected>$pembayaran[lama_rawat]</option>";?>
