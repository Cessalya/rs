...
$hasil = mysqli_query("SELECT kode_ruangan FROM ruangan");

//query untuk mengambil data dari tabel cleaning service
$query = mysqli_query("SELECT MAX(kode_clean_serv) AS kodecs FROM cleaning_service");
$cs = mysqli_fetch_array($query);
...
<form name="postform" method="post" action="isi_cs.php">
  ...
  <tr>
	...
	<?php 
	while ($koderuang = mysqli_fetch_array($hasil)){
