...
$hasil = mysqli_query("SELECT nama_dokter FROM dokter");

//menambil nilai maks dari tabel suster
$query = mysqli_query("SELECT MAX(kode_suster) AS kode_suster FROM suster");
...
<input type="hidden" name="kode_suster" value=<?php echo $kode_suster ?> />
  ...
  <tr>
    ...
    <td><select name="dokter">
	  ...
	  while($dokter = mysqli_fetch_array($hasil)){
