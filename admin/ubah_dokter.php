...
$query = mysqli_query("SELECT nama_dokter FROM dokter WHERE kode_dokter IN (SELECT kode_dokter FROM tgl_masuk WHERE kode_pasien = '$kode_pasien')");
$dokter_awal = mysqli_fetch_array($query);

//mengambil daftar nama dokter
$hasil2 = mysqli_query("SELECT nama_dokter FROM dokter");
$dokter = mysqli_fetch_array($hasil2);
?>
<form name="ubah_dokter" method="post" action="ubah2_dokter.php">
  ...
  <tr>
    ...
    <td><select name="nama_dokter">
	  ...
	  <?php 
	  while ($dokter = mysqli_fetch_array($hasil2)){
